...
public class AdminDashboardController {
    ...
    @FXML private Label lblOrgCount;
    @FXML private Label lblCoordCount;
    ...
    private void loadStatistics() {
        int totalOrgs = 0, totalCoords = 0, totalFamilies = 0, servedFamilies = 0;

        try {
            Connection conn = DatabaseConnection.getInstance().getConnection();
            Statement stmt = conn.createStatement();

            ResultSet rs = stmt.executeQuery("SELECT COUNT(*) FROM tbl_organization");
            if (rs.next()) totalOrgs = rs.getInt(1);

            rs = stmt.executeQuery("SELECT COUNT(*) FROM tbl_user WHERE role = 'COORDINATOR'");
            if (rs.next()) totalCoords = rs.getInt(1);

            rs = stmt.executeQuery("SELECT COUNT(*) FROM tbl_family");
        ...
        if (lblOrgCount != null) lblOrgCount.setText(String.valueOf(totalOrgs));
        if (lblCoordCount != null) lblCoordCount.setText(String.valueOf(totalCoords));
    ...
    @FXML
    public void handleManageOrganizations(ActionEvent event) {
        loadScreen(event, "/views/ManageOrganizations.fxml", "Manage Organizations");
    }

    @FXML
    public void handleManageCoordinators(ActionEvent event) {
        loadScreen(event, "/views/ManageUsers.fxml", "Manage Coordinators");
    }
    ...
    @FXML
    public void handleViewAid(ActionEvent event) {
        loadScreen(event, "/views/ManageAid.fxml", "Aid Distributions");
    }
